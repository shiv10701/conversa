...
function Chat_Search(){
  ...
  useEffect(()=>{
    const res=async()=>{
      ...
      if(result.profile_img){
        url_of_image="https://conversa-3.onrender.com/uploads/"+result._id+"/"+result.profile_img;
      }
      else{
        url_of_image="https://conversa-3.onrender.com/uploads/avatar.jpg"
      }
  ...
  async function change_profile(e){
    ...
    const result_change=await  axios.post('https://conversa-3.onrender.com/api/changeuserdetails/user_profile',{...data},{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
  ...
  async function log_out(){
    ...
    if(user_data!==null){
       try {
         ...
         const response = await axios.get("https://conversa-3.onrender.com/api/auth/log-out",{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
         navigate("/sign-in");
       } catch (error) {
