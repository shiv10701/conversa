...
function Chat_Search(){
  ...
  useEffect(()=>{
    const res=async()=>{
      ...
      if(result.profile_img){
        url_of_image="http://localhost:5000/uploads/"+result._id+"/"+result.profile_img;
      }
      else{
        url_of_image="http://localhost:5000/uploads/avatar.jpg"
      }
  ...
  async function change_profile(e){
    ...
    const result_change=await  axios.post('http://localhost:5000/api/changeuserdetails/user_profile',{...data},{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
  ...
  async function log_out(){
    ...
    if(user_data!==null){
       try {
         ...
         const response = await axios.get("http://localhost:5000/api/auth/log-out",{headers: {'Content-Type': 'multipart/form-data','ngrok-skip-browser-warning': 'true'}});
         navigate("/sign-in");
       } catch (error) {
